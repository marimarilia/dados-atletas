class Atleta {
    ...
    // Método para calcular a média válida das notas (método do projeto anterior)
    calculaMediaValida() {
      let notasOrdenadas = this.notas.sort((a, b) => a - b); // Ordena as notas
      let notasValidas = notasOrdenadas.slice(1, 4); // Elimina a maior e a menor
      let somaNotas = notasValidas.reduce((acc, nota) => acc + nota, 0);
      return somaNotas / notasValidas.length;
    }
  
    // Métodos de acesso para retornar as informações dos atletas
    obtemNomeAtleta() {
      return this.nome;
    }
